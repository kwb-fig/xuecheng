...
public class MyCourseTableServicesImpl implements MyCourseTablesService {
    ...
    public XcChooseCourseDto addChooseCourse(String userId, Long courseId) {
        ...
        if (coursePublish == null) {
            throw new addCourseTeacherException("课程信息不存在");
        }
        Long id = coursePublish.getId();
        if (id == null) {
            throw new addCourseTeacherException("id为空");
        }
        // 课程收费标准
    ...
    public XcCourseTables addCourseTables(XcChooseCourse chooseCourse) {
        ...
        if (!"701001".equals(status)) {
            throw new addCourseTeacherException("选课未成功，无法添加到课程表");
        }
        // 查询我的课程表
    ...
    public PageResult<MyCourseTableItemDto> myCourseTables(MyCourseTableParams params) {
        ...
        PageResult<MyCourseTableItemDto> pageResult = new PageResult<>(myCourseTableItemDtos,total,page,size);
