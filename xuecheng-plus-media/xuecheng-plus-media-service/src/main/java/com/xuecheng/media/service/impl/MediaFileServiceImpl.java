...
import com.xuecheng.base.Exception.addMediaException;
...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    public MediaFiles getFileById(String mediaId) {
        ...
        if (mediaFiles == null) {
            throw new addMediaException("文件不存在");
        }
        String url = mediaFiles.getUrl();
        if (StringUtils.isEmpty(url)) {
            throw new addMediaException("文件还没有转码处理，请稍后预览");
        }
        return mediaFiles;
    }
    ...
    public UploadFileResultDto uploadFile(Long companyId, UploadFileParamsDto uploadFileParamsDto, String localFilePath,String objectName) {
        ...
        if(!result){
            throw new addMediaException("上传文件失败");
        }
        //入库文件信息
        MediaFiles mediaFiles = currentProxy.addMediaFilesToDb(companyId, fileMd5, uploadFileParamsDto, bucket_mediafiles, objectName);
        if(mediaFiles==null){
            throw new addMediaException("文件上传后保存信息失败");
        }
        //准备返回的对象
